...
## Reproducible Phase 4 Infrastructure

Phase 4 adds a fully reproducible three-VM Ubuntu/KVM environment provisioned with **OpenTofu, libvirt, and cloud-init/NoCloud**.
...
The Phase 4 environment was deliberately destroyed and rebuilt entirely from the committed IaC definitions. A clean deployment created **15 OpenTofu-managed resources**, cloud-init completed on all three nodes, all node_exporter targets reported `UP=1`, the application target reported `UP=1`, PostgreSQL was reachable from the web tier, and the final `tofu plan` returned **No changes**.
...
### Phase 4 reproducible environment

- `p4-web-01` — `192.168.140.10` — Python application + node_exporter
- `p4-db-01` — `192.168.140.20` — PostgreSQL + node_exporter
- `p4-monitor-01` — `192.168.140.30` — Prometheus + Alertmanager + node_exporter

Each Phase 4 VM also receives a management interface from the default libvirt network for SSH administration.
...
### Reproduce the Phase 4 environment
...
To remove the Phase 4 environment:
